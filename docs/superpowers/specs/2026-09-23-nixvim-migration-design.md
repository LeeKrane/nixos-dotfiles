...
LazyVim baseline added (the full list; nothing else from LazyVim's options.lua is carried over): `mapleader = " "`, `clipboard = "unnamedplus"`, `undofile = true`, `undolevels = 10000`, `ignorecase`, `smartcase`, `scrolloff = 4`, `sidescrolloff = 8`, `signcolumn = "yes"`, `splitright`, `splitbelow`, `splitkeep = "screen"`, `confirm`, `completeopt = "menu,menuone,noselect"`, `laststatus = 3`, `updatetime = 200`, `timeoutlen = 300`, `pumheight = 10`, `list = true` (LazyVim `listchars`, tab shown subtly because the user indents with tabs), `virtualedit = "block"`, `wildmode = "longest:full,full"`, `sessionoptions = "buffers,curdir,tabpages,winsize,help,globals,skiprtp,folds"`, `foldlevel = 99`, `foldmethod = "expr"`, `foldexpr = "v:lua.vim.treesitter.foldexpr()"`, `foldtext = ""`.
...
- Each hue moves toward the hue of `$primary` by up to 15° (proportional to angular distance, capped).
...
- `colors/dynamic.lua` (shipped via `extraFiles`):
  ...
  2. Call `require("mini.base16").setup({ palette = p, use_cterm = false })`.
...
- Watch the directory `generated/` once with `vim.uv.new_fs_event`. Filter on `filename == "material_colors.scss"`, and never re-arm.
- If the directory does not exist, skip the watcher silently.
- Handler flow:
  - The trailing debounce is 300 ms and resets on each event.
  - The timer callback leaves libuv's fast-event context via `vim.schedule_wrap`, because `vim.cmd`/`vim.api` raise E5560 there.
  - If the file is valid, recompute the palette and apply `dynamic`.
  - If it is empty or invalid, keep the current palette and retry once after 500 ms.
  - If a non-empty file is still invalid after the retry, show one `vim.notify` WARN.
- Close the handle on `VimLeavePre`.

### Fallback

- At startup, with no valid file, use a fixed palette defined in `theme.nix` (catppuccin-frappe base16 values, dark).
- A missing file or directory logs at DEBUG only (machines without illogical-impulse, `nix run` elsewhere, the flake check).
- Invalid content warns once.
...
- `plugins.blink-cmp.settings`:
  ...
  - `keymap.preset = "none"` plus explicit keys (all insert mode), porting the intent of the dead `nvim-cmp.lua`:
    - `<CR>`: accept only if an item is explicitly selected, else newline.
...
- Self-managed LSP clients that bypass `vim.lsp.enable`:
  ...
  - **Java**: `plugins.jdtls`.
    - Server `jdtLanguageServerPackage = pkgs.jdt-language-server`.
    - Per-project workspace dir under `vim.fn.stdpath("cache") .. "/jdtls/" .. project_name`.
    - Started via the module's filetype autocmd (`start_or_attach`).
...
- `plugins.conform-nvim.settings`:
  ...
  - `format_on_save` as raw Lua, returning nil when `vim.g.disable_autoformat` or `vim.b[buf].disable_autoformat` is set, else `{ timeout_ms = 500, lsp_format = "fallback" }`.
- `plugins.lint.lintersByFt` (top-level camelCase option; this module has no `settings`). Lint on `BufWritePost`/`BufReadPost`/`InsertLeave`.
- Formatter and linter binaries go in `extraPackages`.

### Treesitter

`plugins.treesitter` with highlight and indent enabled, and `grammarPackages` from `config.plugins.treesitter.package.builtGrammars`:

`nix lua luadoc luap vim vimdoc query regex bash diff gitcommit git_rebase gitignore markdown markdown_inline typescript tsx javascript jsdoc html css json jsonc yaml toml python rust go gomod gosum gowork java latex bibtex dockerfile`
